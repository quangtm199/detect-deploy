...
class LoadSteaming1:
    def __init__(self,sources="",img_size=640,stride=32):
        ...
        self.imgs, self.fps, self.frames, self.threads = [None] * n, [0] * n, 0, [None] * n
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            fvs=FileVideoStream(s).start()
            self.cap=fvs
            # w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
            # h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
            self.fps[i] = max(fvs.stream.get(cv2.CAP_PROP_FPS) % 100, 0) or 30.0  # 30 FPS fallback
            self.frames = max(int(fvs.stream.get(cv2.CAP_PROP_FRAME_COUNT)), 0) or float('inf') 
    def __iter__(self):
        self.count = 0
        return self
    def __next__(self):
